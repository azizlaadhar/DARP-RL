...
def reinforce (env,
    ...
    while  i_instance< instances: 
        ...
        while i_epoch<epochs:
            ...
            rewards, log_probs, nb_time_window= simulate(max_step, env ,device)
            train_R =   sum([user.status==0 for user in env.users])*100.0  + sum (rewards)
        ...
        scores.append(min(score_epochs))
        costs.append(min(cost_epochs))
        broken_windows.append(min(broken_windows_epochs))
        i_instance+=1
...
if __name__ == "__main__":
    ...
    scores, tests, costs = reinforce(env,device,optimizer, epochs=5, max_step=50, update_baseline=10, relax_window=True)
    ...
    ax1.plot(scores)
    #ax1.plot(tests)
    ax1.set(xlabel="INSTANCE", ylabel="Testing Penalties", title="Total Penalty")   
    plt.savefig(path_plot +'b3-24pen-.pdf')


    fig2, ax2 = plt.subplots(1,1,figsize=(10,10))
    #ax2.plot(scores)
    ax2.plot(costs)
    ax2.set(xlabel="INSTANCE", ylabel="Testing Costs", title="Total Cost")  
    plt.savefig(path_plot +'b3-24cost-.pdf')
    #TEST ENV WITH LOG
