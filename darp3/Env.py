...
class darpenv():
    ...
    def reset (self,instance_number,relax_window=False):
        ...
        self.users = users
        self.users_available = users.copy()
        self.vehicles = vehicles
        self.vehicles_available = vehicles.copy()
    ...
    def get_vehicles_available (self,time):
        self.vehicles_available= [vehicle for vehicle in self.vehicles
                                  if vehicle.route_pred[len(vehicle.route_pred)-1]!=17 or vehicle.max_route_duration==time
                                    ]

    def get_users_available(self):
        for vehicle in self.vehicles:
            route_pred = vehicle.route_pred.copy()
            for user in self.users_available :
                if (user.id in route_pred) :
                    route_pred.remove(user.id)
                    user.flag =1
                    if (user.id in route_pred):
                        user.flag =2
                        self.users_available.remove(user)
        print("users available", len(self.users_available))
    ...
    #         if (user.id in self.vehicles[vehicle_id].user_ride_time.keys() and user.flag!=2):
    #             # 1: being served by the vehicle which performs an action at time step t
    #              temp = self.vehicles[vehicle_id].route_pred.copy()
    #              temp.remove(user.id+1)
    #              if user.id in temp :
    #                  user.flag =2
    #              else:
    #                 user.flag = 1
    ...
    #     if (len(self.vehicles_available)==1):
    #          mask_info.append(0)
    ...
    def get_vehicle_state(self,vehicle_id,time):
        ...
        mask_info = [1 if (user.delivered == True ) else 0 for user in self.users]
    ...
    def step(self,device,state,prediction,vehicle_id):
        ...
        if prediction.item() + 1 in vehicle.route_pred and prediction.item()!= 16 :
            self.users[prediction.item() ].delivered=True
            r=1000
        vehicle.route_pred.append(prediction.item() + 1)
        vehicle.schedule_pred.append(vehicle.next_free_time)


        return - time_penaltie
