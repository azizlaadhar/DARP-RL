...
def simulate(max_step: int, env: darpenv,device ) -> Tuple[List[float], List[float]]:
    ...
    num_finish = 0
    t=0
    while(env.finish(num_finish)== False and t<50):
        t+=1
        next_free_times = [vehicle.next_free_time for vehicle in env.vehicles]
...
def reinforce (env,
    ...
    while  i_epoch< epochs: 
        ...
        """Aggregate rewards"""
        """-sum(rewards) +"""
        train_R =  sum([user.status==2 for user in env.users])*10
